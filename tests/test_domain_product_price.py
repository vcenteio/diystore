...
def test_domain_price_value_decimal_remains_the_same(
    ...
    p = ProductPriceFactory(value=valid_product_price_decimal)
...
def test_domain_price_calculate_no_discount(vat_rate: float):
    vat = VATFactory(rate=vat_rate)
    p: ProductPrice = ProductPriceFactory(vat=vat, discount=None)
